...
import AdsSpendBillingDetails from './AdsSpendBillingDetails';
...
const OverviewBillingAccordionDetails = ({ setActiveTab }) => {
  ...
  const [expandedAdsRoot, setExpandedAdsRoot] = useState(false);

  const adsRootRef = useRef();

  useClickOutside(adsRootRef, () => {
    setExpandedAdsRoot(false);
  });

  const handleChangeAdsRoot = (panel) => (event, newExpanded) => {
    setExpandedAdsRoot(newExpanded ? panel : false);
  };
  return (
    <div>
      <Divider />
      <Grid container sx={{ mt: 3 }}>
        <Grid item xs={12} md={8}>
          ...
          <Accordion
            ref={adsRootRef}
            expanded={expandedAdsRoot}
            onChange={handleChangeAdsRoot(true)}
            sx={{
          ...
          >
            <AccordionSummary aria-controls="panel1d-content" id="panel1d-header">
              <Stack width="100%" direction="row" justifyContent="space-between">
                ...
                >
                  <KeyboardArrowDownIcon /> Ads Spend
                </Typography>
