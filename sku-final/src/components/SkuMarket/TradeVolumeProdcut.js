...
const TradeVolumeProdcut = () => {
  ...
  const logic = `trade_value[gte]=${product?.trade_value}&sku_marketplace[eq]=${product?.sku_marketplace}&_id[ne]=${product?._id}`;

  const fields =
    'brand_en,price_change,sku,sku_marketplace,sku_rank,sku_rate,buy_box_currency,current_price,price_change,all_images,tags';
  const query = `${logic}&page=1&limit=50&fields=${fields}`;
  const { data: productsData, isSuccess } = useGetAllSellerProductsQuery(query, {
    skip: !product,
  });
...
export default TradeVolumeProdcut;
