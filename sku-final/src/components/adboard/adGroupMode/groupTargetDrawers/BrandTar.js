...
const BrandTar = () => {
  const { brandSearched } = useSelector((state) => state.adBoardControl);
  ...
  return (
    <>
     ...
     { brandSearched.map((b) => <Stack key={b.id} sx={{ my: 2 }}>
          <Divider />
          <Stack sx={{ py: 1 }} direction="row" justifyContent="space-between" alignItems="center">
            <Stack>
              <Typography variant="subtitle1" sx={{ px: 1, fontSize: '14px', mb: '5px' }}>
                {b.title}
              </Typography>
            ...
            <IconButton onClick={() => dispatch(selectMCBKSKU(b))}>
              <AddIcon />
            </IconButton>
          </Stack>
          <Divider />
