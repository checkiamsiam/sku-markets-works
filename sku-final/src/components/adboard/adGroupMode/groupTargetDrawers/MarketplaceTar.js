...
const MarketplaceTar = () => {
  const { marketplacesSearched } = useSelector((state) => state.adBoardControl);
  ...
  return (
    <>
      ...
      <Box sx={{ height: '250px', overflow: 'auto' }}>
        {marketplacesSearched.map((m) => <Stack key={m.id} sx={{ my: 2 }}>
          <Divider />
