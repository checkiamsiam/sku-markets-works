...
const AddAlertOnPD = ({ openAddNew, handleCloseNewAdd, sku, alertData }) => {
                ...
                <Grid item xs={6}>
                  {sku && (
                    <>
                      ...
                      >
                        {(type === 'Price Range' || type === 'Price Moves Above' || type === 'Price Moves Below') && sku?.current_price}
                        {(type === 'Stock Range' || type === 'Stock Moves Above' || type === 'Stock Moves Below') && sku?.stock}
                        {(type === 'Stores Range' || type === 'Stores Moves Above' || type === 'Stores Moves Below') && sku?.store}
                        {(type === 'SKU to Store' || type === 'SKU to Fulfillment') && sku?.sku_rate}
                      </Typography>
                      <Typography
                        sx={{
                          px: 1,
