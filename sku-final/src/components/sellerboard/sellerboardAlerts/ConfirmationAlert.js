...
} from 'features/shipment/shipmentAPI';
...
const ConfirmationAlert = ({
  ...
  const [updateShipped] = useUpdateShipmentShippedMutation();
  const [updateRTVAcceptance] = useUpdateShipmentRTVAcceptanceMutation();
  ...
  const successAlrt = async () => {
    if (btnTitle === 'Pay') {
      // Clear the existing cart if any
      dispatch(clearPlan());
      navigate('/checkout', { state: { orders: order } });
    } else if (btnTitle === 'Remove') {
      callback();
      handleClose();
    } else if (btnTitle === 'Delivered') {
      updateShipped(order).then(() => {
        handleSwalAndRefetch();
      });
    } else if (title === 'RTV Confirm') {
      updateRTVAcceptance(order).then(() => {
        handleSwalAndRefetch();
      });
    } else {
      const data = {
