...
import { useUpdateShipmentDeliveredRTVMutation } from 'features/shipment/shipmentAPI';
import { addShipmentItems } from 'features/shipment/shipmentSlice';
import { useEffect, useRef, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import Swal from 'sweetalert2';
import ShipmentDataColumns from '../pending/pick/ShipmentDataColumns';
import ShipmentPendingRows from '../pending/pick/ShipmentPendingRows';
...
function ChildModal({
  ...
  const [updateDeliveredRTV] = useUpdateShipmentDeliveredRTVMutation();
  ...
  const successAlrt = () => {
    if (btnTitle === 'Return') {
      updateDeliveredRTV({
        id: shipment._id,
        orders: shipmentItems,
        reason: rtvReasonRef?.current?.value,
      }).then(() => {
  ...
  const handleOpen = () => {
    dispatch(addShipmentItems({ data: skus }));
    setOpen(true);
  };
  const handleClose = () => {
    setOpen(false);
