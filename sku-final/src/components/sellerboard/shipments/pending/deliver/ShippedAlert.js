...
import { useUpdateShipmentPackedMutation } from 'features/shipment/shipmentAPI';
...
function ChildModal({
  ...
  const [updatePacked] = useUpdateShipmentPackedMutation();
  const awbRef = useRef();
  ...
  const successAlrt = () => {
    ...
    updatePacked({ id: shipmentId, awb: awbRef?.current?.value }).then(() => {
      Swal.fire({
        position: 'center',
        icon: 'success',
        text: text,
