...
import { useUpdateShipmentPickedMutation } from 'features/shipment/shipmentAPI';
import { useState } from 'react';
import Swal from 'sweetalert2';
import {
  shipmentDataColumns,
  shipmentDataRow,
  shipmentPendingDataColumns,
  shipmentPendingDataRow
} from './dataGridData';
...
function ChildModal({
  ...
  const [updatePicked] = useUpdateShipmentPickedMutation();

  const successAlrt = () => {
    updatePicked({ id: shipmentId }).then(() => {
      Swal.fire({
        position: 'center',
        icon: 'success',
        text: text,
