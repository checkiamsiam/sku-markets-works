...
import { Autocomplete, Button, Divider, IconButton, TextField } from '@mui/material';
...
import { useUpdateShipmentPreparingMutation } from 'features/shipment/shipmentAPI';
import { addShipmentItems } from 'features/shipment/shipmentSlice';
import { useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import Swal from 'sweetalert2';
import { shipmentDataColumns, shipmentDataRow } from './dataGridData';
import ShipmentDataColumns from './ShipmentDataColumns';
import ShipmentPendingRows from './ShipmentPendingRows';
...
function ChildModal({
  ...
  const [updatePreparing] = useUpdateShipmentPreparingMutation();
  const shipmentPendingDataColumns = ShipmentDataColumns({
    sku: shipment?.payment?.orders?.length ?? 0,
    qty: shipment?.payment?.totalQty ?? 0,
  });

  const shipmentItems = useSelector((state) => state?.shipment?.items);

  const shipmentPendingDataRows = ShipmentPendingRows({ orders: shipmentItems ?? [] });
  const dispatch = useDispatch();

  const successAlrt = () => {
    let removedItems = shipment?.payment?.orders?.filter(
      ({ orderNo: item }) => !shipmentItems.map(({ orderNo }) => orderNo._id).includes(item._id)
    );
    removedItems = removedItems?.map((obj) => {
      return { ...obj, deductedQty: obj.orderNo.quantity };
    });

    updatePreparing({ id: shipment?._id, orders: shipmentItems, removed: removedItems }).then(
      () => {
        Swal.fire({
          position: 'center',
          icon: 'success',
  ...
  const handleOpen = () => {
    dispatch(addShipmentItems({ data: shipment?.payment?.orders }));
    setOpen(true);
  };
  const handleClose = () => {
    setOpen(false);
              ...
              <Box sx={{ height: 360 }}>
                {delivered ? (
                  ...
                  <ThemeDataGrid
                    ...
                    rows={shipmentPendingDataRows}
                    rowCount={shipment?.payment?.orders?.length ?? 0}
                    page={page}
                    pageSize={pageSize}
                    setPageSize={setPageSize}
                  />
